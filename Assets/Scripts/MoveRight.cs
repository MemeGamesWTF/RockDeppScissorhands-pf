...
public class MoveRight : MonoBehaviour
{
    ...
    void Start()
    {
        clickable = GameObject.Find("ChooseRock").GetComponent<Clickable>();
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (transform.name.Equals("Rock(Clone)") && collision.transform.name.Equals("EScissors(Clone)")) { score++; clickable.SfxToggle(true); }
        if (transform.name.Equals("Paper(Clone)") && collision.transform.name.Equals("ComputerRock(Clone)")) { score++; clickable.SfxToggle(true); }
        if (transform.name.Equals("Scissors(Clone)") && collision.transform.name.Equals("EPaper(Clone)")) { score++; clickable.SfxToggle(true); }

        //This increments the computer score if it wins
        if (transform.name.Equals("Rock(Clone)") && collision.transform.name.Equals("EPaper(Clone)")) { Escore++; clickable.SfxToggle(false); }
        if (transform.name.Equals("Paper(Clone)") && collision.transform.name.Equals("EScissors(Clone)")) { Escore++; clickable.SfxToggle(false); }
        if (transform.name.Equals("Scissors(Clone)") && collision.transform.name.Equals("ComputerRock(Clone)")){ Escore++; clickable.SfxToggle(false); }
